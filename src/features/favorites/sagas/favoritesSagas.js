...
const addToLocalStorage = (beer) => {
  ...
  localStorage.setItem('favoriteBeers', JSON.stringify([...data, beer]));
};
...
export function* addFavoriteWatcher() {
  yield takeEvery(ADD_FAVORITE, addFavoriteWorker);
}
...
export function* removeFavoriteWatcher() {
  yield takeEvery(REMOVE_FAVORITE, removeFavoriteWorker);
}

export default function* favoriteSagas() {
  yield all(
    [addFavoriteWatcher(), removeFavoriteWatcher()],
  );
}
